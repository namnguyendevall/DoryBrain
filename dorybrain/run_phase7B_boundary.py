...
def run_boundary_grid():
    # Boundary High Resolution Sweep
    betas = np.arange(0, 42, 2)
    decays = np.arange(0.80, 1.04, 0.04) # 0.80, 0.84, 0.88, 0.92, 0.96, 1.00
    seeds = 50  # 50 seeds for higher resolution boundary calculation
    ...
    print(f"Total Boundary Grid Points: {len(points)}")
    
    weights = [0.25, 0.25, 0.25, 0.25]
    
    out_dir = "results/phase7B"
    os.makedirs(out_dir, exist_ok=True)
    out_file = os.path.join(out_dir, "landscape_boundary.jsonl")
    progress_file = os.path.join(out_dir, "progress_boundary.json")
    ...
    for beta, decay in points_to_run:
        ...
        if completed_count % 5 == 0 or completed_count == len(points):
