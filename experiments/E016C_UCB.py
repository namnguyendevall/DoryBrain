...
class Actor:
    ...
    """
    def __init__(self, seed: int, batch_size: int = 32, c: float = 2.0):
        self.c = c
        self.N = {}
    ...
    def choose(self, observation: dict) -> str:
        ...
        N_s = sum(self.N[current_state].values())
        
        # Calculate UCB for all valid actions
        best_action = None
        best_ucb = -float('inf')
        
        for a in valid_actions:
            q_val = self.Q[current_state][a]
            n_sa = self.N[current_state][a]
            
            if n_sa == 0:
                ucb_val = float('inf') # Infinite exploration bonus for unvisited actions
            else:
                ucb_val = q_val + self.c * math.sqrt(math.log(N_s + 1) / (n_sa + 1))
