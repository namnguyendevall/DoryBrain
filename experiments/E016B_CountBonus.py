...
class Actor:
    ...
    """
    def __init__(self, seed: int, batch_size: int = 32, beta: float = 1.0):
        self.beta = beta
        self.N = {}
    ...
    def _get_n(self, s, a):
        if s not in self.N: self.N[s] = {act: 0 for act in self.all_actions}
        return self.N[s][a]
    ...
    def choose(self, observation: dict) -> str:
        ...
        if self.last_state is not None and self.last_action is not None:
            ...
            # Ensure last_state is in N
            if self.last_state not in self.N:
                self.N[self.last_state] = {act: 0 for act in self.all_actions}
            self.N[self.last_state][self.last_action] += 1
            r_eff = r + self.beta / ((self.N[self.last_state][self.last_action] + 1) ** 0.5)
            delta = r_eff + self.gamma * max_q - self._get_q(self.last_state, self.last_action)
        ...
        if self.batch_size > 0 and len(self.buffer) > 0:
            ...
            for (s, a, r_b, s_next, valid_a_next, t_b) in batch:
                ...
                n_val = self._get_n(s, a)
                r_eff_b = r_b + self.beta / ((n_val + 1) ** 0.5)
                delta_b = r_eff_b + self.gamma * max_q_next - self._get_q(s, a)
        ...
        if self.rng.random() < epsilon:
            ...
            if current_state not in self.N:
                self.N[current_state] = {act: 0 for act in self.all_actions}
